...
data = fread('result/VarMAT/Sample/RNAvar/mutratiostr.mat')
...
aa = data[tumor_1 >= 0.5* tumor_2 & tumor_1  > 15 & tumor_1 * normal_2  > 1.2* tumor_2 * normal_1 ]
sum(aa[,4]/aa[,5] > aa[,2]/aa[,3],na.rm = TRUE)/(dim(aa)[1]);print(dim(aa)[1])


write.table(aa[,1], file = "result/VarMAT/Pixel/FiltRNAvar/PASS.site.pos.txt", row.names = FALSE, col.names = FALSE, sep="\t", quote = FALSE, append = FALSE)