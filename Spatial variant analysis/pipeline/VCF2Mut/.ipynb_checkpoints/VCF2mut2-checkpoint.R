...
setnames(mutin, c("WT",'mut','location'))
mutin$WT = paste0('_',mutin$WT)
...
read_and_process_file <- function(file_path) {
  ...
  data[, Mutation := NA_real_]
  data[V5 != '', Mutation := str_count(pattern = fixed(mut),V5) / (str_count(pattern = fixed(':'),V5)-1) ]
  data[, location := paste(location, mut, sep = "")]
